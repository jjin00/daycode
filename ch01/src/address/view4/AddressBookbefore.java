...
public class AddressBookbefore extends JFrame {
    ...
    private void initComponents() {
        ...
        font= new Font("D2Coding",font.PLAIN, 12);

		labelTimer = new JLabel("현재 시간");
		labelTimer.setFont(font);
		panelTimer = new JPanel();
		path = "C:\\Users\\andju\\eclipse-workspace\\1\\src\\address\\view4\\images\\";
	...
	// 조회 메뉴나 조회 아이콘 선택시 작업을 정의합니다. ctrl상세조회메소드불러오고
	private void detailActionPerformed() { //deptdao 삭제하기  ㅊㅅ기tkdtpwhghlapthemqnffjdhrh select문 send로 받아오는거  deptdao 상세조회 set
		System.out.println("상세조회 구현");
		int dex[] = new int [vos.length];
		dex = table.getSelectedRows();
...
//				Integer id= (Integer)myTableModel.getValueAt(index[0], 0);
				...
				(myTableModel.getValueAt(dex[0], 0).toString());				
				AddressVO rVo = null;
				for(int i=0;i<vos.length;i++) {
					if(id == vos[i].getId()) {
						rVo = new AddressVO( vos[i].getId(),vos[i].getName(),vos[i].getAddress(),vos[i].getTelephone()
					             ,vos[i].getRelationship(),vos[i].getGender(),vos[i].getBirthday()
					             ,vos[i].getComments(),vos[i].getRegistedate());
					}
				}
				mDialog.set("상세조회", false, rVo,abook); // 확인버튼에서 메세지 뜨는부분 처리
...
//				Integer id= (Integer)myTableModel.getValueAt(index[0], 0);
				...
				for(int i=0;i<vos.length;i++) {
					if(id == vos[i].getId()) {
						newVo = new AddressVO( vos[i].getId(),vos[i].getName(),vos[i].getAddress(),vos[i].getTelephone()
								             ,vos[i].getRelationship(),vos[i].getGender(),vos[i].getBirthday()
								             ,vos[i].getComments(),vos[i].getRegistedate());
					}
				}
...
//		Integer id= (Integer)myTableModel.getValueAt(index[0], 0);
	...
	public void refreshData() {
		...
		vos = ctrl.send();	
		if (vos.length!=0) {
			System.out.println("길이가 0이 아닙니다.");
			...
			for(int i = 0; i<vos.length; i++) {
				...
				oneRow.addElement(vos[i].getGender());
				oneRow.addElement(vos[i].getRelationship());
				oneRow.addElement(vos[i].getBirthday());
				oneRow.addElement(vos[i].getComments());
				oneRow.addElement(vos[i].getRegistedate());
